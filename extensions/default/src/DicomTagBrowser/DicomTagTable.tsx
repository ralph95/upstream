  ...
  return (
    <div
      ...
      className={classNames(
        'hover:bg-secondary-main border-secondary-light text-foreground flex w-full flex-row items-center break-all bg-[rgb(var(--background))] text-base transition duration-300',
        lineHeightClassName
      )}
      key={keyPrefix}
    >
...
function ColumnHeaders({ tagRef, vrRef, keywordRef, valueRef }) {
  return (
    <div
      className={classNames(
        'ohif-scrollbar flex w-full flex-row overflow-y-scroll bg-[rgb(var(--secondary-dark))]'
      )}
    ...
    >
      <div className="w-4/24 px-3">
        <label
          ref={tagRef}
          className="flex flex-1 select-none flex-col pl-1 text-lg text-[rgb(var(--text))]"
        >
...
function DicomTagTable({ rows }: { rows: Row[] }) {
  ...
  return (
    <div>
      ...
      />
      <div
        className="relative m-auto border-2 border-black bg-[rgb(var(--background))]"
        style={{ height: '32rem' }}
      >
        {isHeaderRendered() && (
          <List
